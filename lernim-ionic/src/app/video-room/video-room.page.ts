...
@Component({
    ...
    animations: [trigger("slideLeftRight", [state("in", style({
        ...
        trigger("slideLeftRightChat", [state("in", style({
            transform: "translateX(0px)"
        })),
            state("out", style({
                transform: "translateX(100px)"
            ...
            transition("in => out", animate("200ms", keyframes([style({transform: "translateX(100px)"})]))),
            transition("out => in", animate("200ms", keyframes([style({transform: "translateX(0px)"})])))]),
        trigger("slideTopBottom", [state("in", style({
        ...
        trigger("slideBottomTop", [state("in", style({
            transform: "translateY(0px)",
            visibility: "visible"
        })),
            state("out", style({
...
export class VideoRoomPage implements OnInit, OnDestroy {
    // Constants
    ANDROID_PERMISSIONS = [this.androidPermissions.PERMISSION.CAMERA,
    ...
    constructor(private platform: Platform, private router: Router, private route: ActivatedRoute,
        private openViduSrv: OpenViduService, public modalController: ModalController,
    ...
    public toggleButtonsOrEnlargeStream(event: any) {
        const element: HTMLElement = event.composedPath()
        .filter((e: HTMLElement) => e.className && e.className.includes("OT_root"))[0];
    ...
    private subscribedToStreamDestroyed() {
        this.session.on("streamDestroyed", (event: StreamEvent) => {
            ...
            } else if (this.mainStreamManager.stream.connection.connectionId ===
                ...
                .then(() => {
                    ...
                    .then((camera) => {
                        ...
                        .then((audio) => {
                            ...
                            .catch((err) => {
                                console.error(
                                    "Checking permission " + this.androidPermissions.PERMISSION.MODIFY_AUDIO_SETTINGS +
                                    " failed");
                                reject(err);
                            });
                        })
